...
id=record_crash(k).id;
    ...
    time_detect1=time_detect-time_trace_start1+1;
    time_detect2=time_detect-time_trace_start2+1;
    time_trace_length1=time_trace_end1-time_detect1;
    time_trace_length2=time_trace_end2-time_detect2;
    ...
    totaldist=0;
    for i=0:length-1
        dist=distance(states1(:,time_detect1+i),states2(:,time_detect1+i));
    ...
    p1_crash=states1(1:3,time_detect1)+v1_detect*dur_time;
    p2_crash=states2(1:3,time_detect2)+v2_detect*dur_time;
    ...
    title('速度大小');
    line([time_crash-time_detect time_crash-time_detect],[0 max(max(v1_norm),max(v2_norm))],'color','k');
    %legend('First','Second');
    ...
    title('加速度大小');
    line([time_crash-time_detect time_crash-time_detect],[0 max(max(acc1_norm),max(acc2_norm))],'color','k');
    %legend('First','Second');
    ...
    ylabel('angle(degree)');
    title('速度与上一帧速度的角度');
    line([time_crash-time_detect time_crash-time_detect],[0 max(max(v1_ang),max(v2_ang))],'color','k');
    %legend('First','Second');
    ...
    ylabel('angle(degree)');
    title('加速度与上一帧加速度的角度');
    line([time_crash-time_detect time_crash-time_detect],[0 max(max(acc1_ang),max(acc2_ang))],'color','k');
    %legend('First','Second');
    ...
    line([time_crash-time_detect time_crash-time_detect],[min(min(v1_on_v1_past),min(v2_on_v2_1)) max(max(v1_on_v1_past),max(v2_on_v2_1))],'color','k');
    %legend('First','Second');
    ...
    line([time_crash-time_detect time_crash-time_detect],[min(min(acc1_on_v1_past),min(acc2_on_v2_1)) max(max(acc1_on_v1_past),max(acc2_on_v2_1))],'color','k');
    %legend('First','Second');
    ...
    ylabel('distance(mm)');
    title('果蝇与碰撞点的距离');
    line([time_crash-time_detect time_crash-time_detect],[0 max(max(dist_s1_crash_point),max(dist_s2_crash_point))],'color','k');
    %legend('First','Second');
    ...
    ylabel('distance(mm)');
    title('果蝇间距离');
    line([time_crash-time_detect time_crash-time_detect],[0 max(dist_s1_s2)],'color','k');
    %legend('First','Second');
